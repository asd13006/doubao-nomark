...
# ── 工具 (只有在 Step 1 才顯示，避免打斷生成流程) ──
    if st.session_state.step == 1 and st.session_state.view_mode == "wizard":
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    with _col_left:
        ...
        # Existing materials (收納為進階選項)
        with st.expander("➕ 補充已有材料 (進階/可選)", expanded=False):
            st.markdown("<div style='font-size:12px; color:var(--text-muted); margin-bottom:12px;'>提供已有的靈感，AI 會基於這些素材延伸創作：</div>", unsafe_allow_html=True)
            _mat_placeholders = {
    ...
    with _col_right:
        with st.container(key="tuning_terminal"):
            ...
            _tone_options = {"Catchy (吸睛)": 25, "Balanced (平衡)": 50, "Dramatic (戲劇化)": 75}
            # 找出目前的 key
            _current_tone_name = next((k for k, v in _tone_options.items() if v == st.session_state.prompt_tone), "Balanced (平衡)")
            
            selected_tone = st.radio(
                "Tone", 
                options=list(_tone_options.keys()), 
                index=list(_tone_options.keys()).index(_current_tone_name),
                horizontal=True, 
                label_visibility="collapsed",
                key="tone_radio"
            )
            st.session_state.prompt_tone = _tone_options[selected_tone]
            
            # ── Style Variance (改用明確區間) ──
            st.markdown('<p class="tuner-label" style="margin-top:16px;">Style Variance</p>', unsafe_allow_html=True)
            _style_options = {"Minimal (極簡)": 25, "Dynamic (動態)": 50, "Complex (豐富)": 75}
            _current_style = st.session_state.get("prompt_style_variance", 50)
            _current_style_name = next((k for k, v in _style_options.items() if v == _current_style), "Dynamic (動態)")
            
            selected_style = st.radio(
                "Style", 
                options=list(_style_options.keys()), 
                index=list(_style_options.keys()).index(_current_style_name),
                horizontal=True, 
                label_visibility="collapsed",
                key="style_radio"
            )
            st.session_state["prompt_style_variance"] = _style_options[selected_style]
...
# 原本：h = max(h, 400)
        h = max(h, 400) + 150  # 加上安全餘裕，避免內容被截斷

        with st.expander("📋 詳細檢視（含複製/翻譯按鈕）", expanded=False):
            # 將 scrolling 改為 False
